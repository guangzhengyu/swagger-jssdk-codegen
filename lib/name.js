...
export default function generateApiName (path, method, isSingleResource) {
  return action(method, isSingleResource) + resourceName(path, isSingleResource)
}

function action (method, isSingleResource) {
  if (isSingleResource && ['post', 'put'].indexOf(method)) return 'update'
  return {
    get: 'get',
    post: 'create',
    put: 'update',
